...
public class CompassManager {
	...
	private float high_pass_factor = 0.03f;
		...
		mSensorEventListener = new SensorEventListener() {
			...
			public void onSensorChanged(SensorEvent event) {
				...
				// Gyroscope
					...
					maxAngularSpeed = Math.max(maxAngularSpeed, event.values[0]);
					maxAngularSpeed = Math.max(maxAngularSpeed, event.values[1]);
					maxAngularSpeed = Math.max(maxAngularSpeed, event.values[2]);
					// keep compass pointing to magnet north in the first 0.5s
					if (period > 1000)
					{
						if(maxAngularSpeed >= 5){
							initValues();
							return;
						}
				...
				// rotation vector
					...
					if(firstRotate || startTime % 30000 < 4){
